...
<html>
   ...
   <% request.setCharacterEncoding("UTF-8"); %>
	...
	<sql:setDataSource var="snapshot" driver="com.mysql.cj.jdbc.Driver"
     ...
     <sql:query dataSource="${snapshot}" var="stu">
     ...
     <c:forEach var="row" items="${stu.rows}">
     	<% bl=false; %>
          ...
          <% String ept= request.getParameter("sid"); %>
     <%if(ept==""){%>
     	<script language='javascript'>
     		alert('学号不能为空，请重新输入！');
     ...
     <sql:update dataSource="${snapshot}" var="result">
		insert into student
			(sid,name,sex,age,phone,mail,password,card_id,mid,classid)
		values (?<sql:param value="${param.sid}" />,
