      ...
      <script src="js/bootstrap.min.js"></script>
		<ul class="nav nav-tabs">
 		<li><a href="student.jsp">个人信息</a></li>
  		...
  		<li class="active"><a href="student_remove.jsp">退课</a></li>
	...
	<sql:setDataSource var="snapshot" driver="com.mysql.cj.jdbc.Driver"
     ...
     <c:forEach var="row" items="${choose_switch.rows}">
     	<c:if test="${row.the_switch=='off'}">
     		<script language='javascript'>
				window.location.href="student_remove_falsetime.jsp";
			</script>
     	</c:if>
     </c:forEach>
...
<div class="container">
 <form name="input" action="student_remove.jsp" method="POST">
 	<input type="text" name="cid" placeholder="请输入课序号" value="${param.cid}"/>
 	<input type="submit" value="搜索课程" class="btn btn-default"/>
 </form>
 <form name="input" action="student_remove_act.jsp" method="POST">                          
	<table class="table table-hover">
		...
		<tbody>
		 <c:forEach var="row" items="${choosed.rows}" varStatus="Status">
	...
	<input type="submit" value="退选课程" class="btn btn-default" style="float:right">
 </form>
</div>
