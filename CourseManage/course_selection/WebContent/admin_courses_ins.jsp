...
<html>
   ...
   <% request.setCharacterEncoding("UTF-8"); %>
    ...
    <sql:query dataSource="${snapshot}" var="result">
     ...
     <% boolean bl=true,empty=false; %>
     <c:forEach var="row" items="${result.rows}">
     	<% bl=false; %>
     </c:forEach>
     <% String ept= request.getParameter("cid"); %>
     <%if(ept==""){%>
     	<script language='javascript'>
     		alert('课程号不能为空，请重新输入！');
			window.location.href="admin_courses.jsp";
		</script>
     <%}else if(bl==false){%>
     	<script language='javascript'>
     		alert('已存在该课程，插入失败！');
     ...
     <sql:update dataSource="${snapshot}" var="result">
		insert into course(cid,cname,cpreid,ccredit,ctime,mid)
		values(
			?<sql:param value="${param.cid}" />,
			?<sql:param value="${param.cname}" />,
			?<sql:param value="${param.cpreid}" />,
			?<sql:param value="${param.ccredit}" />,
			?<sql:param value="${param.ctime}" />,
