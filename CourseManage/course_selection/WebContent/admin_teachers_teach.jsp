<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@ page import="java.io.*,java.util.*,java.sql.*"%>
<%@ page import="javax.servlet.http.*,javax.servlet.*" %>
<%@ taglib uri="http://java.sun.com/jsp/jstl/core" prefix="c"%>
<%@ taglib uri="http://java.sun.com/jsp/jstl/sql" prefix="sql"%>

<!DOCTYPE html>
<html>
   <head>
      <title>山东大学选课管理系统-管理端</title>
      <meta name="viewport" content="width=device-width, initial-scale=1.0">
      <link href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css" rel="stylesheet">
      <script src="https://oss.maxcdn.com/libs/html5shiv/3.7.0/html5shiv.js"></script>
      <script src="https://oss.maxcdn.com/libs/respond.js/1.3.0/respond.min.js"></script>
   </head>
   <body>
   <% request.setCharacterEncoding("UTF-8"); %>
      <c:if test="${user==NULL}">
   	  	<script language='javascript'>
			window.location.href="index.jsp";
		</script>
   	  </c:if>
      <script src="https://code.jquery.com/jquery.js"></script>
      <script src="js/bootstrap.min.js"></script>
      
		<ul class="nav nav-tabs">
 		<li><a href="admin.jsp">个人信息</a></li>
  		<li><a href="admin_students.jsp">学生信息</a></li>
  		<li class="active"><a href="admin_teachers.jsp">教师信息</a></li>
  		<li><a href="admin_courses.jsp">课程信息</a></li>
  		<li><a href="admin_departments.jsp">院系信息</a></li>
  		<li style="float:right"><a href="logout.jsp">退出登录</a></li>
		</ul>
		
	<sql:setDataSource var="snapshot" driver="com.mysql.cj.jdbc.Driver"
     url="jdbc:mysql://localhost:3306/dikzoku?useUnicode=true&serverTimezone=UTC&characterEncoding=utf-8"
     user="root"  password="123456"/>
     
    <sql:query dataSource="${snapshot}" var="teached">
     	select tid,course.cid,cname,ccredit,ctime from teachcourse,course 
     	where tid=?<sql:param value="${param.tid}" />
     		and teachcourse.cid=course.cid
     </sql:query>
     
<div class="container">                        
	<table class="table table-hover">
		<thead>
			<h4><c:out value="${param.tname}(${param.tid}) 所授课程"/></h4>
			<tr>
				<td>课程号</td>
				<td>课程名</td>
				<td>学分</td>
				<td>学时</td>
			</tr>
		</thead>
		<tbody>
		 <c:forEach var="row" items="${teached.rows}">
		  <tr>
   		   <td><c:out value="${row.cid}"/></td>
   		   <td><c:out value="${row.cname}"/></td>
   		   <td><c:out value="${row.ccredit}"/></td>
   		   <td><c:out value="${row.ctime}"/></td>
   		  </tr>
		 </c:forEach>
		</tbody>
	</table>
	<div>
	<form action="admin_teachers_teach_ins.jsp">
		<input type="hidden" name="tid" value="${param.tid}"/>
		<input type="hidden" name="tname" value="${param.tname}"/>
		<input type="text" name="cid" placeholder="请输入课程号"/>
		<button type="submit" class="btn btn-default" >添加授课信息</button>
	</form>
	</div>
	<div>
	<form action="admin_teachers_teach_del.jsp">
		<input type="hidden" name="tid" value="${param.tid}"/>
		<input type="hidden" name="tname" value="${param.tname}"/>
		<input type="text" name="cid" placeholder="请输入课程号"/>
		<button type="submit" class="btn btn-default" >删除授课信息</button>
	</form>
	</div>
	<div>
	<form action="admin_teachers.jsp">
	<button type="submit" class="btn btn-default" style="float:right;">返回</button>
	</form>
	</div>
</div>
     
     
   </body>
</html>