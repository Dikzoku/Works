   ...
   <% request.setCharacterEncoding("UTF-8"); %>
	...
	<sql:setDataSource var="snapshot" driver="com.mysql.cj.jdbc.Driver"
     ...
     <sql:update dataSource="${snapshot}" var="result">
		UPDATE course
		SET cname=?<sql:param value="${param.cname}" />,
			cpreid=?<sql:param value="${param.cpreid}" />,
			ccredit=?<sql:param value="${param.ccredit}" />,
			ctime=?<sql:param value="${param.ctime}" />,
			mid=?<sql:param value="${param.mid}" />
		WHERE course.cid = ? <sql:param value="${param.cid}" />;
	</sql:update>
