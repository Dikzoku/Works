...
<html>
   ...
   <% request.setCharacterEncoding("UTF-8"); %>
    ...
    <sql:query dataSource="${snapshot}" var="teached">
     ...
     <form name="input" action="teacher_enter_scores.jsp" method="POST">
...
<div class="container" style="width:600px"> 
 ...
 <form name="input" action="teacher_enter_scores_act.jsp" method="POST">                           
	<table class="table table-hover">
		...
		<tbody>
		 ...
		 <c:forEach var="row" items="${teached_students.rows}">
		 	...
		 	<%i++;%>
		 	<input type="hidden" name="sid<%=i%>" value="${row.sid}"/>
		  <tr>
   		   <td><c:out value="${row.sid}"/></td>
   		   <td><c:out value="${row.name}"/></td>
   		   <c:if test="${row.score!=null}">
   		   	<td><input type="text" class="form-control" name="score<%=i%>" placeholder="${row.score}"/></td>
   		   </c:if>
   		   <c:if test="${row.score==null}">
   		   	<td><input type="text" class="form-control" name="score<%=i%>"/></td>
   		   </c:if>
   		  </tr>
   		  </c:if>
		 </c:forEach>
	...
	<input type="hidden" name="courseid" value="${param.courseid}"/>
	<button type="submit" class="btn btn-default" style="float:right;">录入成绩</button>
 </form> 
</div>
