...
const int PixLength=5;
float graylevel[w/PixLength+1][h/PixLength+1];
float weight[PixLength+1][PixLength+1];
...
void Setweight(){
    ...
    for(int i=0;i<PixLength;i++){
        for(int j=0;j<PixLength;j++){
            ...
            float maxdis= (0.5-1.0*PixLength/2)*(0.5-1.0*PixLength/2)+(0.5-1.0*PixLength/2)*(0.5-1.0*PixLength/2) ;
            float dis=(x-1.0*PixLength/2)*(x-1.0*PixLength/2)+(y-1.0*PixLength/2)*(y-1.0*PixLength/2) ;
                        weight[i][j]=maxdis+1-dis;
            sum+=weight[i][j];
            }
...
void DrawPoly(){
    ...
    for(int i=0;i<=w/PixLength;i++){
        for(int j=0;j<=h/PixLength;j++){
	        ...
	        glVertex2f(i*PixLength,j*PixLength);
        }
    }
    glEnd();
}
